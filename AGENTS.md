...
- Focused run: `python -m unittest tests.test_message_rag_services` or
 ...
 the 18 test files hold 45 `TestCase` classes, all named `<Subject>Test`).
...
now 187 in 18 files** (`b5851ab` added `tests/test_repo_hygiene.py`, `6f1dc79` added
...
- `docs/BOT_SYSTEM_REPORT.md` cites `pytest -q` and 9 tests; both runner and count are wrong
 (187 stdlib `unittest` tests across 18 files). It now carries a staleness banner listing its
