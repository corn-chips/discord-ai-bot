...
| 3 | DAB-078 | `search_recent` cannot use its index; add the two `created_at DESC` indexes | IMPROVEMENT | 1200x / 1870x measured | XS | **16.0** | — | [DAB-078](analysis-tickets/DAB-078.md) |
| 4 | DAB-212 | `get_pending_embeddings` ORDER BY rewrite | IMPROVEMENT | 1844x measured, two lines | XS | **16.0** | — | [DAB-212](analysis-tickets/DAB-212.md) |
...
| 7 | DAB-095 | `sqlite_utils` sets no PRAGMAs: no WAL, implicit busy timeout, FKs off | IMPROVEMENT | 5.01 s failure -> 3.3 ms; 4.2x writes | S | **9.0** | — | [DAB-095](analysis-tickets/DAB-095.md) |
...
| 29 | DAB-066 | Legacy migration silently discards all rows, then records success | BUG | **S1** | S | **4.0** | — | [DAB-066](analysis-tickets/DAB-066.md) |
