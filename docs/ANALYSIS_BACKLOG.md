...
defects; the other two, DAB-065 and DAB-066, are in Tier 1. See the S1 table in
...
| 15 | DAB-198 | **PARTIAL** (Phase 1, `4fb091c`) — PDF resource bomb: a 3.6 KB upload could cost 107 s of CPU and returned nothing. The per-page pixel clamp landed and closes the S1 as filed (5.8 s, 20 pages returned, re-measured at `083ebea`); **seven of the eight acceptance criteria did not**. There is still no total-megapixel budget, no wall-clock timeout, no `attachment.size` gate and no failure-streak break, so the same fixture retains 792 MP at 3.3 GB peak and hands all of it to a paid request — `PPR-21`, and criterion 6 ("no 1.6 GB spike") is unmet. **This lands first**, and DAB-197's PNG round-trip deletion follows it in the same commit — the reverse order removes the only guard against a 256 MP allocation ([`ANALYSIS_CORRECTIONS.md`](ANALYSIS_CORRECTIONS.md) item 6) | BUG | **S1** | S | **6.0** | — (blocks DAB-197) | [DAB-198](analysis-tickets/DAB-198.md) |
...
| 26 | DAB-073 | **PARTIAL** (Phase 2/3, `36bf261`) — a retrieval floor is reserved in `available_retrieval_slots`, the site [`ANALYSIS_CORRECTIONS.md`](ANALYSIS_CORRECTIONS.md) item 7 names as correct, so the index is no longer left unsearched. Pins still have absolute priority *inside* the assembled pack: acceptance criterion 2 fails on execution (20 pins at `max_messages=8` packs 0 non-pin items), criteria 3-6 are untaken, and the floor made a billed rerank reachable at exactly that shape — `PPR-20` | BUG | S2 | S | **6.0** | — | [DAB-073](analysis-tickets/DAB-073.md) |
...
| 31 | DAB-042 | **PARTIAL, and UNBLOCKED** (round 2, Phases 0 and 1). Backoff on every retry decision landed in `0283c5b`; the whole-sequence deadline did not. No whole-sequence deadline yet, but DAB-204 has landed and a correct 480 s deadline now leaves the suite green. Read the ticket header first: the budget must go **inside** `GeminiClient`, and this ticket's acceptance criterion contradicts DAB-204's unless it does. The 488 s applies only to `medium`/`high`; two adjacent findings outrank it (an untimed router `to_thread`, and four billed calls in 0.0001 s on empty-STOP) | BUG | S2 | M | **3.0** | — (DAB-204 discharged) | [DAB-042](analysis-tickets/DAB-042.md) |
...
- **`AGENTS.md` contained one verified error, since corrected.** It stated `_pin_service` is
  ...
  `083ebea`; the ones in *this* document and in the ticket bodies were not, and 58 implementation
  commits have
  moved most of them.
...
round 2 at the commit each names. **Nine of the twenty-one have since been fixed** and say so in
...
### PPR-21 (S3) — DAB-198's clamp is per page, so nothing bounds the attachment as a whole
