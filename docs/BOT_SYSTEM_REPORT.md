...
> |---|---|---|
> | 16 | "`python -m pytest -q` passed: **9 tests**." | Wrong on both counts. pytest is not configured anywhere (no `pyproject.toml`, `setup.cfg`, `pytest.ini`, or `tox.ini`; pytest appears in neither `requirements.in` nor `constraints.txt`), and async tests would error without `pytest-asyncio`. The suite is **394 stdlib `unittest` tests in 37 files** at HEAD `083ebea` (125 in 13 files when this banner was written at `c83f740`), run from the repository root with `python -m unittest discover -s tests -p "test_*.py"`. |
