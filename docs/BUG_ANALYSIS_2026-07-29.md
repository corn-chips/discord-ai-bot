...
| D | **DAB-068** | **CLOSED (round 2 Phase 3, `0fb7016` + `01f75ba`; the empty-table half only on 2026-08-06). This description is backwards** — the migration wrote its ledger unconditionally, so it burned its one shot and never retried; "retries on every boot forever" was the sibling defect. Round 2 armed the *absent* legacy table and left the *empty* one burning, which is the ordinary pre-split state; both are armed now. Pin legacy migration fails silently and retries on every boot forever | `pin_service.py:55-70` |
