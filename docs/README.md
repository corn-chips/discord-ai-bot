...
- **CURRENT** - measured against branch `dev`, HEAD `c83f740`, on 2026-07-29. Act on it. Two
  commits have landed since (`0bf532c` dead-code removal, `0c9eb91` dependency drop). The findings
  themselves remain valid, but every `file:line` reference into a file those commits touched has
  shifted. Trust the finding and re-locate the symbol; do not trust the line number.
...
| [`tech-debt-register.md`](tech-debt-register.md) | The technical debt register. Eight resolved items with re-measured evidence, plus ten items (TD-009 to TD-018) added on 2026-07-29, of which TD-015 (dead code) was closed by `0bf532c`/`0c9eb91`, leaving nine open. Uses a `(impact x frequency) / effort` priority score. | CURRENT | 2026-07-29 |
| [`tech-debt-sprints/`](tech-debt-sprints/) | The four phased plans that closed TD-001 to TD-008, plus the roadmap README. A record of how those items were retired, not a live plan. Some figures inside are stale: the gate count of 98 tests is now 125, and after `0bf532c` the `DiscordBot` class is 1,067 lines not 1,131, with `BotConfig` at 96 fields not 93. Sprint 1's TD-004 project-layout claim was false when written; the README has since been repaired and that criterion now holds. | HISTORICAL | 2026-07-12 / 2026-07-13 |
...
HEAD `c83f740`, using `.venv/bin/python` (CPython 3.12.13). Two commits have landed since:
`0bf532c` (dead-code removal, -2,111 lines across 25 files) and `0c9eb91` (`constraints.txt`
