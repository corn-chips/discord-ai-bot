...
- **CURRENT** - measured against branch `dev`, HEAD `c83f740`, on 2026-07-29. Act on it. Four
  commits have landed since (`9894bcc` dead-code removal, `4baa29c` dependency drop, then
  `b5851ab` and `6f1dc79`, both test-only). The findings themselves remain valid, but every
  `file:line` reference into a file `9894bcc` touched has shifted. Trust the finding and
...
| [`tech-debt-register.md`](tech-debt-register.md) | The technical debt register. Eight resolved items with re-measured evidence, plus ten items (TD-009 to TD-018) added on 2026-07-29, of which TD-015 (dead code) was closed by `9894bcc`/`4baa29c`; TD-019 was then registered for the incoherence `9894bcc` exposed, leaving ten open. Uses a `(impact x frequency) / effort` priority score. | CURRENT | 2026-07-29 |
| [`tech-debt-sprints/`](tech-debt-sprints/) | The four phased plans that closed TD-001 to TD-008, plus the roadmap README. A record of how those items were retired, not a live plan. Some figures inside are stale: the gate count of 98 tests is now 140 in 15 files, and after `9894bcc` the `DiscordBot` class is 1,067 lines not 1,131, with `BotConfig` at 96 fields not 93. Sprint 1's TD-004 project-layout claim was false when written; the README has since been repaired and that criterion now holds. | HISTORICAL | 2026-07-12 / 2026-07-13 |
...
`9894bcc` (dead-code removal, -2,111 lines across 25 files), `4baa29c` (`constraints.txt`
57 -> 51 pins), and `b5851ab` / `6f1dc79`, which are test-only. Their findings still hold, but
`file:line` evidence pointing into a file `9894bcc` touched no longer resolves; re-locate the
...
15 files at HEAD, after `b5851ab` added `tests/test_repo_hygiene.py` (2) and `6f1dc79` added
