...
Last updated: 2026-07-29 (re-measured at branch `dev`, HEAD `4baa29c`)

Open items: 10 | Resolved items: 9 | Total: 19
Original Sprint 1-4 estimate: 1 XL + 2 L + 5 M | Current open estimate: 2 S + 4 M + 4 L

Every figure in this document was re-measured on 2026-07-29 against `4baa29c` using
`.venv/bin/python` (CPython 3.12.13). The figures were first taken at `c83f740` and re-taken
after `9894bcc` (dead-code removal) and `4baa29c` (five dependencies dropped) landed. Prior
